...
class WindowScores(Toplevel): 
    ...
    def __init__(self, master = None): 
        ...
        Label(self, text =texte).pack(padx=20, pady=10)
        Label(self, text ="\n10 meilleurs scores : ",  font = ("Arial", 14, "bold")).pack() 
        Label(self, text =str(MAXscores)).pack()
        Button(self, text="Fermer les scores", command=self.destroy).pack(pady=10)
...
def callback(event):
    ...
    mastermind.prend_joueur(username)
    mastermind.main_game()
...
btnquit = Button(master, text ="Quitter le jeu ...")
btnquit.bind("<Button>", quit_game)
btnquit.pack(pady = 5)
